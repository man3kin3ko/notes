...
В этих же документах подчеркивается, что основным параметром пароля, обеспечивающим его криптостойкость, является его длина, и рекомендуется **не мучать пользователя сложными парольными политиками**. В организациях, где происходит принудительная ротация паролей, можно встретить подобные мутации: `Mypassword1!`
...
Помимо горизонтального и вертикального разграничения доступа, иногда выделяют контекстное разграничение доступа - то есть ограничение к пройденному или недоступному шагу многоступенчатой последовательности. Типичные уязвимости могут включать недостаток авторизации на определенных этапах или использование заголовка `Referer` для определения последовательности шагов пользователя вместо сохранения этой информации в сессии. Во время прохождения двухфакторной аутентификации пользователь может иметь валидную сессию до первой попытки ввода кода.
## Федерация
